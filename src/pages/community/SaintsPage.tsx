...
const SaintsPage: React.FC = () => {
  ...
  const selectedSaint = saintId 
    ? allSaints.find(saint => saint.id === saintId) || null
    : null;

  const handleSaintSelect = (saint: Saint) => {
    navigate(`/community/saints/${saint.id}`, { replace: true });
  };

  const handleClosePanel = () => {
    navigate('/community/saints', { replace: true });
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <SaintsList 
        ...
        onSaintSelect={handleSaintSelect}
