...
const LibraryPage: React.FC = () => {
  ...
  return <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-card rounded-lg shadow-md py-px px-0">
        <div className="flex flex-col md:flex-row gap-4 mb-8 p-6">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
            <Input type="text" placeholder="Search books..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
          
          <div className="flex gap-2 overflow-x-auto pb-2">
            {categories.map(cat => <Button key={cat} variant={category === cat ? "default" : "outline"} size="sm" onClick={() => setCategory(cat)} className={category === cat ? "bg-dominican-burgundy hover:bg-dominican-burgundy/90" : "border-dominican-burgundy text-dominican-burgundy hover:bg-dominican-burgundy/10"}>
        ...
        <div className="p-6 pt-0">
          {loading ? <div className="flex justify-center py-10">
              ...
              {filteredBooks.map(book => <div key={book.id} className="border border-dominican-light-gray dark:border-border rounded-lg overflow-hidden flex flex-col bg-white dark:bg-card">
                  <AspectRatio ratio={1 / 1.5} className="bg-dominican-light-gray dark:bg-muted">
                    {book.coverImage ? <img src={book.coverImage} alt={book.title} className="object-cover h-full w-full" onError={e => {
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      ...
                      <div className="flex gap-2">
                        {isAdmin && <Button asChild variant="outline" size="sm" className="border-dominican-burgundy text-dominican-burgundy hover:bg-dominican-burgundy/10" title="Edit book">
                            <Link to={`/admin/books/edit/${book.id}`}>
                              <Edit className="h-4 w-4" />
                            </Link>
                          </Button>}
                        <Button asChild variant="outline" size="sm" className="border-dominican-burgundy text-dominican-burgundy hover:bg-dominican-burgundy/10" disabled={!book.epubPath && canReadBooks} title={!book.epubPath ? "No digital version available" : undefined} onClick={() => handleBookClick(book)}>
              ...
              {filteredBooks.length === 0 && !loading && <div className="col-span-full text-center py-10">
                  <p className="text-gray-600 dark:text-gray-400">No books found matching your search criteria.</p>
                </div>}
            </div>}
        </div>
        
        {!canReadBooks && <div className="m-6 p-4 bg-dominican-burgundy/10 dark:bg-dominican-burgundy/20 rounded-md">
      ...
      {/* Admin controls */}
      {isAdmin && <div className="mt-6 bg-white dark:bg-card rounded-lg shadow-md p-6">
